...
def test_none_status_unknown():
    r = explain(None, CFG)
    assert r["state"] == "unknown" and "nicht verfügbar" in r["headline"]

def test_paused():
    r = explain(s(mode="paused"), CFG)
    assert r["headline"] == "Pausiert (Not-Aus)"

def test_manual_on_off():
    assert explain(s(mode="manual", relay_on=True), CFG)["headline"] == "WP manuell EIN"
    assert explain(s(mode="manual", relay_on=False), CFG)["headline"] == "WP manuell AUS"

def test_auto_waiting_surplus_shows_streak():
    r = explain(s(reason="waiting_surplus", on_streak=2, on_delay_cycles=3), CFG)
...
def test_auto_waiting_min_offtime_countdown():
    r = explain(s(reason="waiting_min_offtime", secs_since_off=600, min_offtime_s=900), CFG)
...
def test_auto_on_min_runtime_countdown():
    r = explain(s(relay_on=True, reason="min_runtime", secs_since_on=1320, min_runtime_s=1800), CFG)
...
def test_auto_on_surplus_ok_shows_compensated():
    ...
    r = explain(s(relay_on=True, reason="surplus_ok", surplus_w=285), CFG)
    assert r["headline"] == "WP läuft"
...
def test_auto_on_net_draw_with_off_countdown():
    ...
    r = explain(s(relay_on=True, reason="surplus_ok", surplus_w=-400,
                  off_streak=2, off_delay_cycles=3), CFG)
...
def test_auto_stale_state_shows_failsafe():
    r = explain(s(relay_on=False, state_fresh=False, state_age_s=47, reason="state_stale_failsafe"), CFG)
...
def test_fresh_state_unaffected():
    ...
    r = explain(s(relay_on=False, state_fresh=True, on_streak=2), CFG)
